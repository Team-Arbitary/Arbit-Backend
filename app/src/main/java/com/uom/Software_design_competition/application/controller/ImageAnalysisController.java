...
public class ImageAnalysisController extends BaseController {
        ...
        @Operation(summary = "Trigger manual thermal analysis", description = "Initiates AI-powered thermal image analysis for a specific inspection and transformer. Requires both baseline and thermal images to be uploaded first.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Trigger manual analysis (simplified)", description = "Initiates analysis using only inspection number. Automatically retrieves transformer number from inspection record.")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Analysis triggered successfully"),
        ...
        @Operation(summary = "Get analysis result", description = "Retrieves thermal image analysis results for a specific inspection including anomaly detection data and thermal overlay image.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Update analysis result JSON", description = "Updates the analysis result JSON after frontend modifications. Used to save user-edited anomaly states or analysis data.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<ApiResponse<AnalysisResult>> updateAnalysisResultJson(
                        @Parameter(description = "Inspection number", example = "6") @PathVariable String inspectionNo,
                        @Parameter(description = "Transformer number", example = "TX-1345") @PathVariable String transformerNo,
                        @Parameter(description = "Updated analysis result JSON", example = "[{\"id\":\"1\",\"anomalyState\":\"Faulty\"}]") @RequestBody String analysisResultJson,
                        HttpServletRequest request) throws BaseException {
                long startTime = System.currentTimeMillis();
                log.info(LoggingAdviceConstants.REQUEST_INITIATED, request.getMethod(), request.getRequestURI());
                log.info("Received update request for inspection: {}, transformer: {}", inspectionNo, transformerNo);
                ApiResponse<AnalysisResult> resp = imageAnalysisService.updateAnalysisResultJson(inspectionNo,
                                transformerNo,
                                analysisResultJson);
                log.info(LoggingAdviceConstants.REQUEST_TERMINATED, System.currentTimeMillis() - startTime,
                                resp.getResponseDescription());
        ...
        @Operation(summary = "Get analysis results by transformer", description = "Retrieves all analysis results for a specific transformer across multiple inspections.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<ApiResponse<List<AnalysisResult>>> getAnalysisResultsByTransformer(
                ...
                long startTime = System.currentTimeMillis();
                log.info(LoggingAdviceConstants.REQUEST_INITIATED, request.getMethod(), request.getRequestURI());
                ApiResponse<List<AnalysisResult>> resp = imageAnalysisService
                                .getAnalysisResultsByTransformer(transformerNo);
                log.info(LoggingAdviceConstants.REQUEST_TERMINATED, System.currentTimeMillis() - startTime,
                                resp.getResponseDescription());
                return setResponseEntity(resp);
        }
